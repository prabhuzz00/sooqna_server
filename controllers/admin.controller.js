...
export async function authWithGoogle(request, response) {
  ...
  try {
    ...
    if (!existingUser) {
      ...
      await AdminModel.findByIdAndUpdate(existingUser?._id, {
        last_login_date: new Date(),
      });
...
export async function loginUserController(request, response) {
  try {
    ...
    if (email) {
      ...
      user = await AdminModel.findOne({ phone: phone });
...
export async function getAllUsers(request, response) {
  try {
    ...
    const totalUsers = await AdminModel.find();
    ...
    const total = await AdminModel.countDocuments(users);
...
export async function deleteUser(request, response) {
  const user = await AdminModel.findById(request.params.id);
  ...
  const deletedUser = await AdminModel.findByIdAndDelete(request.params.id);
